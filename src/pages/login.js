...
function UserForm(props) {
  ...
  function HandleClick(event) {
    ...
    if (props.type === "register") {
      ...
      axios
        .post(
          "https://thepc-bknd.onrender.com/api/users/signup",
          qs.stringify(regInfo),
          {
            headers: {
        ...
        .then((response) => {
          if (response.status === 201) {
            ...
            SaveCredentials(
              ...
              response.data.newUser.name,
              response.data.token,
              response.data.newUser.onboarding
            );
            setSuccess(2);
          }
      ...
      axios
        .post(
          "https://thepc-bknd.onrender.com/api/users/login",
          qs.stringify(logInfo),
          {
            headers: {
              "Content-type": "application/x-www-form-urlencoded",
            },
        ...
        .then((response) => {
          ...
          if (response.status === 200) {
            console.log(response.data);
            SaveCredentials(
              response.data.userFound._id,
              response.data.userFound.name,
              response.data.token,
              response.data.userFound.onboarding
            );
            setSuccess(2);
          }
          if (response.status === 400) {
                  ...
                  <div className="col-md-9 col-lg-8 mx-auto">
                    {props.type === "login" && (
                      <>
                        <h1>
                          {" "}
                          <LockFill className="mb-2 display-4" />
                        </h1>
                        <h1 className="login-heading mb-4">
                          <strong>
                            {logInfo.email
                              ? _.truncate(logInfo.email, {
                                  separator: /@/,
                                  length: 18,
                                })
                              : "Hey, it's you again!"}
                        ...
                        {success === 200 && (
                          <h4 className="text-success mb-4">
                            Sign in successful! {logInfo.email}
                          </h4>
                        )}
                        <form>
                          ...
                          <div className="form-label-group text-center">
                            ...
                            >
                              &copy; 2023 THEPC - VIT. All Rights Reserved.{" "}
                              <br /> v2.0.b0.1
                            </p>
                          </div>
                        </form>
                        ...
                        <form>
                          ...
                          <div className="form-label-group">
                            <input
                              ...
                              onChange={HandleChange}
                            />
                            <label htmlFor="inputPasswordCnf">
                              Confirm Password
                            </label>
                          </div>
